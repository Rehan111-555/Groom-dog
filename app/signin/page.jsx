...
/* ================================
   HEADER + NAV (centered logo, right tools, hover mega)
   ================================ */
function AppHeader() {
  ...
  return (
    <header className="w-full sticky top-0 z-50">
      ...
      <div className="bg-[var(--header-top-bg)] text-[var(--header-top-fg)] transition-colors">
        {/* NOTE: large gap-x pushes search/tools further to the RIGHT away from the centered logo */}
        <div className="max-w-[1280px] mx-auto px-4 lg:px-6 h-[92px] grid grid-cols-[1fr_auto_1fr] items-center gap-x-28 md:gap-x-36 xl:gap-x-48 relative">
          {/* Left: phone */}
          ...
          {/* Right: search + tools (stays at top-right) */}
      ...
      {/* 1-inch breathing space between top row and navbar */}
      ...
      <nav className="bg-[#2f2f2f] text-[#d7d7d7] border-t border-black/10">
        <div className="max-w-[1280px] mx-auto px-2 lg:px-4 relative" onMouseLeave={()=>setOpen(null)}>
  ...
  return (
    <main className="min-h-screen flex flex-col bg-[var(--page-bg)] text-[var(--page-fg)] transition-colors">
                ...
                {mode==='signup' && (
                  <div className="mb-4">
                    ...
                    <input type="text" value={name} onChange={(e)=>setName(e.target.value)}
                      className="w-full h-[50px] rounded-[10px] px-4 ring-1 ring-gray-300 focus:ring-2 focus:ring-[#6b6bff] outline-none" required/>
                  </div>
                )}
      ...
      <style jsx global>{`
        ...
        :root{
          --joyzze-teal:#1cd2c1;
          --page-bg:#f4f6f9; --page-fg:#0e0f11; --muted-fg:#6b7280;
