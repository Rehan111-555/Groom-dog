...
/* ---------------- Theme ---------------- */
const BRAND = {
  charcoal: '#1f2937',       // main charcoal
  charcoalHover: '#111827',  // darker hover
  charcoalBorder: '#374151', // border shade
};
...
function CompareSlider({ beforeSrc, afterSrc }) {
  ...
  return (
    <div className="relative h-full w-full rounded-2xl overflow-hidden bg-slate-50 select-none" style={{ touchAction: 'none' }}>
      ...
      <div className="absolute top-0 bottom-0" style={{ left: `${pos}%`, width: 2, background: 'rgba(31,41,55,0.95)' }} />
...
function validateImageFile(f, maxMB){
  ...
  if (!String(f.type||"").startsWith("image/")) return "Please upload an image file.";
  if (f.size > MAX*1024*1024) return `Image too large. Keep it under ${MAX}MB.`;
  return null;
}
...
function UploadAndResult(){
  ...
  const groom=async()=>{
    ...
    try{
      const form=new FormData(); form.append("image",file); form.append("dog_only","true");
      ...
      const data=await res.json(); const url=pickResultUrl(data); if(!url) throw new Error("Bad response.");
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600">Upload → Groom → Compare</p>
          </div>
        </div>
        {resultUrl && <a className="btn btn-charcoal" href={resultUrl} download><Icon.Download/> Download</a>}
      </div>

      <div className="grid lg:grid-cols-2 gap-8">
        <Card className="p-4">
          {!previewUrl ? (
            <label className="grid place-items-center rounded-2xl border border-dashed border-slate-300 cursor-pointer hover:bg-white" style={{ height: panelH }}>
              <div className="grid place-items-center gap-3">
                <div className="w-14 h-14 rounded-2xl bg-white grid place-items-center shadow"><Icon.Upload/></div>
                <div className="font-medium">Drag &amp; drop or click</div>
              ...
              <div className="mt-3 flex gap-3">
                {!loading ? (
                  <>
                    ...
                    <Button className="btn-outline-charcoal" onClick={()=>setFile(null)}><Icon.Reset/> Reset</Button>
                  </>
                ) : (
        ...
        <Card className="p-4">
          ...
          <div className="rounded-2xl overflow-hidden" style={{ height: panelH }}>
            {resultUrl ? <CompareSlider beforeSrc={previewUrl} afterSrc={resultUrl}/> :
              <div className="h-full grid place-items-center border border-dashed border-slate-300 bg-slate-50/60 text-sm text-slate-600">
    ...
    <header className="relative overflow-hidden text-white" style={{ backgroundColor: BRAND.charcoal }}>
      <div className="container mx-auto px-6 py-20 grid lg:grid-cols-2 gap-10 items-center">
        <div>
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold">Make your dog look freshly groomed—<span className="text-slate-300">with AI</span></h1>
          <p className="mt-4 text-slate-300 max-w-xl">Upload a photo → AI tidies fur → Compare before & after.</p>
        ...
        <div className="rounded-3xl overflow-hidden shadow-2xl">
          <img src="/dog-4.jpg" alt="Hero" className="w-full h-auto object-cover"/>
        </div>
      </div>
    </header>
  );
