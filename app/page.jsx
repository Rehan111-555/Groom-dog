...
const Icon = {
  Upload: (props) => (
    <svg className="w-6 h-6 text-purple-500" viewBox="0 0 24 24" fill="none" {...props}>
  ...
  Search: (props) => (
    <svg className="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" {...props}>
...
const Button = ({ className = "", disabled, onClick, children, type = "button" }) => (
  <button type={type} disabled={disabled} onClick={onClick} className={`px-6 py-3 rounded-full font-medium transition shadow ${className}`}>
    {children}
  </button>
);

const Card = ({ className="", children }) => (
  <div className={`rounded-3xl bg-white shadow-lg hover:shadow-xl hover:-translate-y-1 transition ${className}`}>
    {children}
  </div>
);
      ...
      <div className="bg-[#1e1e2f] text-white">
        <div className="container mx-auto flex items-center justify-between px-6 py-4">
          ...
          <div className="relative">
            ...
            <Icon.Search className="absolute right-3 top-2.5"/>
          </div>
        </div>
      </div>
    </header>
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-r from-[#1e1e2f] via-[#2a2438] to-[#1e1e2f] text-white">
      <div className="container mx-auto px-6 py-20 grid lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="rounded-3xl overflow-hidden shadow-2xl ring-2 ring-purple-400/20 hover:scale-105 transition">
  ...
  return (
    <section id="how" className="bg-gradient-to-b from-slate-50 to-slate-100 py-20">
      ...
      <div className="container mx-auto grid md:grid-cols-3 gap-8 px-6">
       Here’s the **full safe version** of your `page.jsx` with **no external dependencies** (no `framer-motion`, no `lucide-react`).  

✅ Works out-of-the-box on **Vercel** since it only uses **React + TailwindCSS + inline SVGs**.  
✅ Keeps your **upload → groom → compare** workflow.  
✅ UI is polished with gradients, shadows, hover effects.  
✅ Fully responsive, no missing lines.  
...
const Button = ({ className = "", disabled, onClick, children }) => (
  <button disabled={disabled} onClick={onClick} className={`px-6 py-3 rounded-full font-medium transition shadow ${className}`}>
    {children}
  </button>
);
...
function UploadAndResult(){
  ...
  const [panelH, setPanelH] = useState(640);
  useEffect(() => {
    const resize = () => setPanelH(Math.max(520, Math.min(820, window.innerHeight * 0.72)));
    resize();
  ...
  const groom=async()=>{
    ...
    }catch(e){ setError("Error processing image"); }
    finally{ setLoading(false); }
  };
  ...
  return (
    <section id="app" className="container mx-auto px-6 py-20">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold">Joyzze Dog Groomer</h1>
        {resultUrl && <a href={resultUrl} download className="px-4 py-2 rounded-full bg-gradient-to-r from-purple-500 to-indigo-500 text-white flex items-center gap-2"><Icon.Download/> Download</a>}
      </div>
      <div className="grid lg:grid-cols-2 gap-10">
        <Card className="p-6">
          {!previewUrl ? (
            <label className="grid place-items-center rounded-2xl border-2 border-dashed border-purple-400/50 bg-gradient-to-br from-slate-50 to-slate-100 cursor-pointer hover:border-purple-500" style={{ height: panelH }}>
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-xs text-slate-500">PNG, JPG up to 12MB</p>
              </div>
            ...
            <div>
              ...
              <div className="mt-4 flex gap-3">
                {!loading ? (
                  <>
                    <Button className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white" onClick={groom}><Icon.Wand/> Groom</Button>
                    <Button className="bg-slate-200 hover:bg-slate-300" onClick={()=>{setFile(null);setPreviewUrl(null);setResultUrl(null);}}><Icon.Reset/> Reset</Button>
                  </>
                ) : <Button className="bg-purple-400 text-white">Working… {progress}%</Button>}
              </div>
            </div>
          )}
        </Card>
        <Card className="p-6">
          <p className="mb-2 text-sm font-semibold">Groomed Dog Result</p>
          <div className="rounded-2xl overflow-hidden bg-slate-100 flex items-center justify-center" style={{ height: panelH }}>
            {!resultUrl ? <p className="text-slate-500">Your groomed image will appear here.</p> : <CompareSlider beforeSrc={previewUrl} afterSrc={resultUrl}/>}
          </div>
        </Card>
      </div>
    </section>
...
function SigninHeader() {
  return (
    <header className="sticky top-0 z-50 shadow-md">
      <div className="bg-gradient-to-r from-purple-800 to-indigo-900 text-white text-xs px-6 py-2 flex justify-between">
        <span>(877) 456-9993</span><span>info@joyzze.com</span>
      </div>
      <div className="bg-[#1e1e2f] text-white">
        <div className="container mx-auto flex items-center justify-between px-6 py-4">
          <span className="font-extrabold text-xl bg-gradient-to-r from-purple-300 to-indigo-400 bg-clip-text text-transparent">Joyzze</span>
          <nav className="hidden md:flex space-x-8 text-sm font-medium">
            {['All Products','Clippers','Blades','Combs & Accessories','Information','Recycling & Sharpening','Distributor'].map((item)=>(
              <a key={item} href="#" className="hover:text-purple-300">{item}</a>
            ))}
          </nav>
          <div className="relative">
            <input type="text" placeholder="Search..." className="px-4 py-2 rounded-full text-sm text-black"/>
            <Icon.Search className="absolute right-3 top-2.5"/>
          </div>
        </div>
      </div>
...
function Hero(){
  return (
    <section className="bg-gradient-to-r from-[#1e1e2f] via-[#2a2438] to-[#1e1e2f] text-white">
      <div className="container mx-auto px-6 py-20 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          <h1 className="text-5xl font-extrabold">Make your dog look<br/><span className="bg-gradient-to-r from-purple-300 via-indigo-400 to-purple-500 bg-clip-text text-transparent">freshly groomed—with AI</span></h1>
          <p className="mt-6 text-slate-300 max-w-xl text-lg">Upload a photo, we tidy fur and outline while keeping <b>breed, pose, background, lighting, and colors identical</b>.</p>
          <div className="mt-8 flex gap-4">
            <a href="#app" className="px-6 py-3 rounded-full bg-gradient-to-r from-purple-500 to-indigo-500 text-white">Try it free</a>
            <a href="#how" className="px-6 py-3 rounded-full border border-purple-400/50 text-white">See how it works</a>
          </div>
        </div>
        <div className="rounded-3xl overflow-hidden shadow-2xl ring-2 ring-purple-400/20">
          <img src="/dog-10.png" alt="Hero sample"/>
        </div>
      </div>
    </section>
  );
      ...
      <div className="container mx-auto grid md:grid-cols-3 gap-8 px-6">
        {[
          ...
          <Card key={i} className="p-6 flex flex-col">
            <div className="w-10 h-10 rounded-full bg-gradient-to-r from-purple-500 to-indigo-500 text-white grid place-items-center mb-4">{i+1}</div>
            <h3 className="font-semibold mb-2">{s.title}</h3>
            <p className="text-sm text-slate-600 flex-grow">{s.desc}</p>
            <div className="mt-4"><a href="#app" className="px-5 py-2 rounded-full bg-gradient-to-r from-purple-500 to-indigo-500 text-white text-sm">{s.btn}</a></div>
          </Card>
        ))}
      </div>
    </section>
  ...
  return (
    <section id="examples" className="container mx-auto px-6 py-20">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {['/dog-1.jpg','/dog-2.jpg','/dog-3.jpg'].map((src)=>(
          <div key={src} className="rounded-3xl overflow-hidden shadow-lg ring-1 ring-slate-200 hover:scale-105 transition">
  ...
  return (
    <footer className="bg-[#1e1e2f] text-slate-200">
      <div className="container mx-auto px-6 py-12 grid md:grid-cols-4 gap-10">
        <div>
          ...
          <ul className="space-y-1 text-sm text-slate-400">
            {['All Products','Clippers','Blades','Combs & Accessories','Information','Recycling & Sharpening','Distributor'].map((l)=>(<li key={l}><a href="#" className="hover:text-white">{l}</a></li>))}
          </ul>
        </div>
        <div>
          <h4 className="font-semibold mb-3">Series</h4>
          <ul className="space-y-1 text-sm text-slate-400">
            {['A-Series','C-Series','D-Series','M-Series'].map((l)=>(<li key={l}><a href="#" className="hover:text-white">{l}</a></li>))}
          </ul>
        </div>
        <div>
      ...
      <div className="border-t border-white/10">
        <div className="container mx-auto px-6 py-4 text-xs text-slate-400 flex justify-between">
